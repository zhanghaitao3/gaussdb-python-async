...
cdef gaussdb_uuid_bytes_from_str(str u, char *out):
    cdef:
        const char *orig_buf
        Py_ssize_t size
        unsigned char ch
...
cdef gaussdb_uuid_from_buf(const char *buf):
    cdef:
...
cdef class UUID(__UUIDReplaceMe):
    ...
    def __init__(self, inp):
        ...
        if cpython.PyBytes_Check(inp):
            ...
            gaussdb_uuid_bytes_from_str(inp, self._data)
        else:
...
# <hack>
# In order for `isinstance(gaussdbproto.UUID, uuid.UUID)` to work,
