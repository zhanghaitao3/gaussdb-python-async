...
cdef hstore_encode(CodecContext settings, WriteBuffer buf, obj):
    ...
    for k, v in items:
        ...
        as_gaussdb_string_and_size(settings, k, &str, &size)
        item_buf.write_int32(<int32_t>size)
        item_buf.write_cstr(str, size)
        if v is None:
            item_buf.write_int32(<int32_t>-1)
        else:
            as_gaussdb_string_and_size(settings, v, &str, &size)
            item_buf.write_int32(<int32_t>size)
            item_buf.write_cstr(str, size)
...
cdef hstore_decode(CodecContext settings, FRBuffer *buf):
    ...
    for i in range(elem_count):
        ...
        k = decode_gaussdb_string(settings, frb_read(buf, elem_len), elem_len)

        elem_len = hton.unpack_int32(frb_read(buf, 4))
        if elem_len < 0:
            v = None
        else:
            v = decode_gaussdb_string(settings, frb_read(buf, elem_len), elem_len)
