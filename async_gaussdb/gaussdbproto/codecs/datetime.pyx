...
gaussdb_epoch_datetime = datetime.datetime(2000, 1, 1)
cdef int32_t gaussdb_epoch_datetime_ts = \
    <int32_t>cpython.PyLong_AsLong(int(gaussdb_epoch_datetime.timestamp()))

gaussdb_epoch_datetime_utc = datetime.datetime(2000, 1, 1, tzinfo=utc)
cdef int32_t gaussdb_epoch_datetime_utc_ts = \
    <int32_t>cpython.PyLong_AsLong(int(gaussdb_epoch_datetime_utc.timestamp()))

gaussdb_epoch_date = datetime.date(2000, 1, 1)
cdef int32_t gaussdb_date_offset_ord = \
    <int32_t>cpython.PyLong_AsLong(gaussdb_epoch_date.toordinal())

# Binary representations of infinity for datetimes.
cdef int64_t gaussdb_time64_infinity = 0x7fffffffffffffff
cdef int64_t gaussdb_time64_negative_infinity = <int64_t>0x8000000000000000
cdef int32_t gaussdb_date_infinity = 0x7fffffff
cdef int32_t gaussdb_date_negative_infinity = <int32_t>0x80000000
...
cdef inline _encode_time(WriteBuffer buf, int64_t seconds,
    ...
    if ts == infinity_datetime_ts:
        buf.write_int64(gaussdb_time64_infinity)
    elif ts == negative_infinity_datetime_ts:
        buf.write_int64(gaussdb_time64_negative_infinity)
    else:
        buf.write_int64(ts)
...
cdef inline int32_t _decode_time(FRBuffer *buf, int64_t *seconds,
    ...
    if ts == gaussdb_time64_infinity:
        return 1
    elif ts == gaussdb_time64_negative_infinity:
        return -1
    else:
...
cdef date_encode(CodecContext settings, WriteBuffer buf, obj):
    cdef:
        ...
        int32_t gaussdb_ordinal

    if ordinal == infinity_date_ord:
        gaussdb_ordinal = gaussdb_date_infinity
    elif ordinal == negative_infinity_date_ord:
        gaussdb_ordinal = gaussdb_date_negative_infinity
    else:
        gaussdb_ordinal = ordinal - gaussdb_date_offset_ord

    buf.write_int32(4)
    buf.write_int32(gaussdb_ordinal)
...
cdef date_encode_tuple(CodecContext settings, WriteBuffer buf, obj):
    ...
    gaussdb_ordinal = obj[0]
    buf.write_int32(4)
    buf.write_int32(gaussdb_ordinal)


cdef date_decode(CodecContext settings, FRBuffer *buf):
    cdef int32_t gaussdb_ordinal = hton.unpack_int32(frb_read(buf, 4))

    if gaussdb_ordinal == gaussdb_date_infinity:
        return infinity_date
    elif gaussdb_ordinal == gaussdb_date_negative_infinity:
        return negative_infinity_date
    else:
        return date_from_ordinal(gaussdb_ordinal + gaussdb_date_offset_ord)


cdef date_decode_tuple(CodecContext settings, FRBuffer *buf):
    cdef int32_t gaussdb_ordinal = hton.unpack_int32(frb_read(buf, 4))

    return (gaussdb_ordinal,)
...
cdef timestamp_encode(CodecContext settings, WriteBuffer buf, obj):
    ...
    delta = obj - gaussdb_epoch_datetime
    cdef:
...
cdef timestamp_decode(CodecContext settings, FRBuffer *buf):
    ...
    if inf > 0:
        ...
        return gaussdb_epoch_datetime.__add__(
...
cdef timestamptz_encode(CodecContext settings, WriteBuffer buf, obj):
    ...
    delta = utc_dt - gaussdb_epoch_datetime_utc
    cdef:
        int64_t seconds = cpython.PyLong_AsLongLong(delta.days) * 86400 + \
                                cpython.PyLong_AsLong(delta.seconds)
        int32_t microseconds = <int32_t>cpython.PyLong_AsLong(
...
cdef timestamptz_decode(CodecContext settings, FRBuffer *buf):
    ...
    if inf > 0:
        ...
        return gaussdb_epoch_datetime_utc.__add__(
            timedelta(0, seconds, microseconds))
...
cdef timetz_encode(CodecContext settings, WriteBuffer buf, obj):
    ...
    # and the UTC, whereas in GaussDBSQL it's the opposite,
