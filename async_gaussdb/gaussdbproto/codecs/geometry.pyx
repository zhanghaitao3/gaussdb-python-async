...
cdef inline _decode_points(FRBuffer *buf):
    ...
    for i in range(npts):
        ...
        point = gaussdbproto_types.Point(x, y)
...
cdef box_decode(CodecContext settings, FRBuffer *buf):
    ...
    return gaussdbproto_types.Box(
        gaussdbproto_types.Point(high_x, high_y),
        gaussdbproto_types.Point(low_x, low_y))
...
cdef line_decode(CodecContext settings, FRBuffer *buf):
    ...
    return gaussdbproto_types.Line(A, B, C)
...
cdef lseg_decode(CodecContext settings, FRBuffer *buf):
    ...
    return gaussdbproto_types.LineSegment((p1_x, p1_y), (p2_x, p2_y))
...
cdef point_decode(CodecContext settings, FRBuffer *buf):
    ...
    return gaussdbproto_types.Point(x, y)
...
cdef path_encode(CodecContext settings, WriteBuffer wbuf, obj):
    ...
    if cpython.PyTuple_Check(obj):
        ...
        is_closed = 0
    elif isinstance(obj, gaussdbproto_types.Path):
...
cdef path_decode(CodecContext settings, FRBuffer *buf):
    ...
    return gaussdbproto_types.Path(*_decode_points(buf), is_closed=is_closed == 1)
...
cdef poly_decode(CodecContext settings, FRBuffer *buf):
    return gaussdbproto_types.Polygon(*_decode_points(buf))
...
cdef circle_decode(CodecContext settings, FRBuffer *buf):
    ...
    return gaussdbproto_types.Circle((center_x, center_y), radius)
