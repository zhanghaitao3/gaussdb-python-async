...
# defined in GaussDBsql/src/include/inet.h
#
DEF GAUSSDBSQL_AF_INET = 2  # AF_INET
DEF GAUSSDBSQL_AF_INET6 = 3  # AF_INET + 1
...
cdef inline int8_t _ver_to_family(int32_t version):
    if version == 4:
        return GAUSSDBSQL_AF_INET
    else:
        return GAUSSDBSQL_AF_INET6
...
cdef net_decode(CodecContext settings, FRBuffer *buf, bint as_cidr):
    ...
    if family != GAUSSDBSQL_AF_INET and family != GAUSSDBSQL_AF_INET6:
...
cdef inet_encode(CodecContext settings, WriteBuffer buf, obj):
    ...
    try:
        ...
        # GaussDBSQL accepts *both* CIDR and host values
