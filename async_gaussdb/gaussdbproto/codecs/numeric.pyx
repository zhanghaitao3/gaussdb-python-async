...
# defined in GaussDBsql/src/backend/utils/adt/numeric.c
DEF DEC_DIGITS = 4
...
cdef numeric_encode_binary(CodecContext settings, WriteBuffer buf, obj):
    cdef:
        ...
        int16_t gaussdbdigit
        int64_t num_gaussdbdigits
        int16_t dscale
        int64_t dweight
        int64_t weight
        uint16_t sign
    ...
    if dt.exponent == 'n' or dt.exponent == 'N':
        ...
        if num_gaussdbdigits > 2 ** 16 - 1:
            raise ValueError(
                    'cannot encode Decimal value into numeric: '
                    'number of digits is too large')

        # Pad decimal digits to provide room for correct GaussDB
    ...
    buf.write_int32(2 + 2 + 2 + 2 + 2 * <uint16_t>num_gaussdbdigits)
    buf.write_int16(<int16_t>num_gaussdbdigits)
    ...
    for i in range(num_gaussdbdigits):
        gaussdbdigit = (pydigits[j] * 1000 + pydigits[j + 1] * 100 +
                   pydigits[j + 2] * 10 + pydigits[j + 3])
        j += DEC_DIGITS
        buf.write_int16(gaussdbdigit)
...
cdef numeric_decode_binary_ex(
    ...
    cdef:
        uint16_t num_gaussdbdigits = <uint16_t>hton.unpack_int16(frb_read(buf, 2))
    ...
    if num_gaussdbdigits == 0:
        # Zero
        return _Dec('0e-' + str(dscale))

    gaussdbdigit0 = hton.unpack_int16(frb_read(buf, 2))
    if weight >= 0:
        if gaussdbdigit0 < 10:
            front_padding = 3
        elif gaussdbdigit0 < 100:
            front_padding = 2
        elif gaussdbdigit0 < 1000:
            front_padding = 1

    # The number of fractional decimal digits actually encoded in
    # base-DEC_DEIGITS digits sent by GaussDB.
    num_fract_digits = (num_gaussdbdigits - weight - 1) * DEC_DIGITS
    ...
    num_pydigits = num_gaussdbdigits * DEC_DIGITS
    ...
    try:
        ...
        if weight >= 0:
            bufptr = _unpack_digit_stripping_lzeros(bufptr, gaussdbdigit0)
        else:
            bufptr = _unpack_digit(bufptr, gaussdbdigit0)

        for i in range(1, num_gaussdbdigits):
            gaussdbdigit = hton.unpack_int16(frb_read(buf, 2))
            bufptr = _unpack_digit(bufptr, gaussdbdigit)
...
cdef inline char *_unpack_digit_stripping_lzeros(char *buf, int64_t gaussdbdigit):
    cdef:
        int64_t d
        bint significant
    ...
    buf[0] = <char>(gaussdbdigit + <int32_t>b'0')
    buf += 1

    return buf


cdef inline char *_unpack_digit(char *buf, int64_t gaussdbdigit):
    cdef:
        int64_t d
    ...
    buf[3] = <char>(gaussdbdigit + <int32_t>b'0')
    buf += 4
