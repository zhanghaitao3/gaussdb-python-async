...
cdef inline as_gaussdb_string_and_size(
...
cdef inline decode_gaussdb_string(CodecContext settings, const char* data,
                             ssize_t len):
...
cdef text_decode(CodecContext settings, FRBuffer *buf):
    ...
    return decode_gaussdb_string(settings, frb_read_all(buf), buf_len)
